          ...
          <tbody className="divide-y divide-gray-100">
            {orders.map((order) => (
              <tr key={order.id} className="text-sm hover:bg-gray-50 transition">
                <td className="px-6 py-4 font-mono font-bold text-blue-600">{order.code}</td>
                <td className="px-6 py-4 text-gray-500">
                  {new Date(order.createdAt).toLocaleString("vi-VN")}
                </td>
                <td className="px-6 py-4 font-medium">{order.staffName}</td>
                <td className="px-6 py-4">{order.itemCount}</td>
                <td className="px-6 py-4 font-bold">{order.totalPrice.toLocaleString()}đ</td>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-black uppercase tracking-tighter ${
